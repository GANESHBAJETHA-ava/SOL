  ...
  <v-app id="game">
  	<div v-if="game != null && period != null">
	    ...
	    <v-dialog v-model="orderDialog" width="600">
	      <v-card>
      		<v-card-title small v-if="order.selection == 'head'" class="blue d-flex justify-space-between" style="color: #fff;">
      			<span>Select Head</span>
      			<v-btn style="font-size: 22px !important;" icon dark @click="orderDialog = false">
	            <v-icon>mdi-close</v-icon>
	          </v-btn>
	        </v-card-title>
	        <v-card-title small v-else-if="order.selection == 'tail'" class="red d-flex justify-space-between" style="color: #fff;">
	        	<span>Select Tail</span>
      			<v-btn style="font-size: 22px !important;" icon dark @click="orderDialog = false">
	            <v-icon>mdi-close</v-icon>
	          </v-btn>
	        </v-card-title>
		    ...
		    <div class="d-flex justify-content-between align-items-center pb-5" style="padding: 0 15px;">
		    	...
		    	<v-dialog v-model="dialog" width="1200">
			      ...
			      <v-card>
			        <v-card-title class="text-h5 grey lighten-2">Rule of Guess</v-card-title>
			        <v-card-text class="mt-3">
			          <p>30 seconds 1 issue, 25 seconds  to order, 5 seconds to show the lottery result. It opens all day. The total number of trade is 28880 issues</p>
			          <p>If you spend 100 to trade, after deducting 2 service fee, your contract amount is 98:</p>
			          <p>1. JOIN HEAD : if the result shows HEAD you will get (98*2) 196</p>
			          <p>2. JOIN TAIL : if the result shows TAIL you will get (98*2) 196 That’s Simple to earn money</p>
			        </v-card-text>
			        <v-divider></v-divider>
			        <v-card-actions>
			          <v-spacer></v-spacer>
	  		...
	  		<v-container style="background-color: whitesmoke;">
	        	...
	        	<v-col>
	  			...
	  			<div class="coin-game-main-block">
	  				<div class="sound-block float-right">
	  					<v-btn fab small color="secondary" icon v-if="soundOn" @click="soundOn = false"><v-icon>mdi-volume-high</v-icon></v-btn>
	  					<v-btn fab small color="secondary" icon v-else @click="soundOn = true"><v-icon>mdi-volume-mute</v-icon></v-btn>
	  				</div>
	  				<div class="floor">
					    <div class="line"></div>
					    <div class="line"></div>
					  ...
					  <div class="coin" :class="{ 'spining': coinSpining }">
					    <div class="edge">
  				...
  				<div class="d-flex flex-row justify-content-around pt-2 pb-3" style="position: relative;">
	        	<div class="bet-lock-overlay" v-if="is_bet_lock"><v-icon>mdi-lock-outline</v-icon></div>
						<v-btn color="blue" dark large fab @click="selectOrder('head')">HEAD</v-btn>
						<v-btn color="red" dark large fab @click="selectOrder('tail')">TAIL</v-btn>
  				</div>
	  		</v-container>
	      ...
	      <div class="details-block mt-5">
	          ...
	          >
		          <template v-slot:item.number="{ item }">
		          	<span>{{ item.number.toUpperCase() }}</span>
					    </template>
	          	<template v-slot:item.result="{ item }">
					      <v-icon color="blue" v-if="item.number == 'head'">mdi-star</v-icon>
					      <v-icon color="red" v-if="item.number == 'tail'">mdi-star</v-icon>
					    </template>
	          </v-data-table>
	        </v-card>
	        <v-card class="mb-10">
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        ...
        headers: [
          ...
          { text: 'Result', align: 'center', value: 'number' },
          { text: '', align: 'center', value: 'result' },
        ],
        allPeriods: [],
        myOrders: [],
    ...
    methods: {
      ...
      setupAnimForCoin(result) {
  			this.coinSpining = false;
      	...
      	//declare objects to animate, remove any existing anim
			  ...
			  // console.log(winner);
			  document.querySelector(':root').style.setProperty('--flips', winner);
			  //apply animation and wait for completion
			  setTimeout(() => {
			    objs.forEach((line) => line.classList.add('anim'));
			  });
			  if (this.soundOn) {
			  	var sourceFlip = window.location.origin + '/sounds/coin-flip.mp3';
					var audioFlip = new Audio(); // use the constructor in JavaScript, just easier that way
					audioFlip.addEventListener("load", function() {
					  audioFlip.play();
					}, true);
					audioFlip.src = sourceFlip;
					audioFlip.autoplay = true;
					audioFlip.play();
			  }
				setTimeout(function () {
				  if (this.soundOn) {
				  	var sourceDrop = window.location.origin + '/sounds/coin-drop.mp3';
						var audioDrop = new Audio();
						audioDrop.addEventListener("load", function() {
						  audioDrop.play();
						}, true);
						audioDrop.src = sourceDrop;
						audioDrop.autoplay = true;
						audioDrop.play();
				  }
				}.bind(this), 4000);
			  setTimeout(function () {
					this.coinSpining = true;
					this.coinFlipping = false;
			  }.bind(this), 6000);
      },
      getGame(init = true) {
      	this.period_countdown = null;
      	axios.get('/get-game/coin-flip').then(res => {
      ...
      echoSetup() {
	      ...
	      Echo.channel(`GameResult.${this.game.id}`).listen('GameResult', (data) => {
	      	if (this.period.id == data.period_id) {
	      		this.setupAnimForCoin(data.result);
	      	}
 	      });
      },
      minusContractCount() {
...
.coin.spining {
	animation: spining 35s linear infinite alternate;
}
...
.coin.spining:after {
  transform: translateZ(-12.5px) rotateY(180deg) rotateZ(0);
}
.coin .edge {
...
@keyframes flip {
  ...
  10% {
    transform: rotateY(45deg) rotateX(calc(var(--flips) / 5)) scale(1.5);
  }
  70% {
    transform: rotateY(-30deg) rotateX(calc(var(--flips) / 3)) scale(2);
  }
  100% {
...
@keyframes spining {
	...
	100% { transform: rotateY(12600deg) }
}
</style>	